 ...
 **/
let getPoolStatsData = null
function getPoolStats(callback) {
   const nextupdate = Date.now() - 1000

   if(!getPoolStatsData || !getPoolStatsData || getPoolStatsData.updated < nextupdate) {
	   try{
	    apiInterfaces.pool('/stats', function(error, data) {
        	if (error) {
	            log('error', logSystem, 'Unable to get API data for stats: ' + error);
       		 } 
		getPoolStatsData = data	
		getPoolStatsData.updated = Date.now()		 
	        callback(error, data);
	    });
	   }catch(e){
		callback(null, getPoolStatsData);
	   }
    } else {
	callback(null,getPoolStatsData);
    }
...
function getNetworkDifficulty(callback) {
    getPoolStats(function(error, stats) {
        callback(error, stats.pool ? stats.network.difficulty : 0);
    });
}
