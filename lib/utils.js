...
exports.minerSeenWithIPForAddress=function(address, ip, callback) {
    ...
    redisClient.hget([config.coin + ':workers_ip:' + address, ip], function(error, result) {
...
export.diffBuffer = bignum('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 16);
// Set nonce pattern - must exactly be 8 hex chars
export.noncePattern = new RegExp("^[0-9A-Fa-f]{8}$");