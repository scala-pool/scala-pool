...
const verifyConfigData = (configData, dont_save) => {
	...
	checkField(configData,'payments');


	if(configData.payments.enabled) {

		checkField(configData.payments,'interval');
		checkField(configData.payments,'maxAddresses');
		checkField(configData.payments,'mixin');
		checkField(configData.payments,'priority');
		checkField(configData.payments,'transferFee');
		checkField(configData.payments,'dynamicTransferFee');
		checkField(configData.payments,'minerPayFee');
		checkField(configData.payments,'minPayment');
		checkField(configData.payments,'networkFee');
		checkField(configData.payments,'minPaymentExchangeAddress');
		checkField(configData.payments,'maxTransactionAmount');
		checkField(configData.payments,'denomination');
	
	}


	checkField(configData,'unlocker');

	if(configData.unlocker.enabled) {

		checkField(configData.unlocker,'interval');
		checkField(configData.unlocker,'depth');
		checkField(configData.unlocker,'reward');
	}
