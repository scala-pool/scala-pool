...
class ModuleSpawner {
	...
	static spawn(mod, cfg){
		if(!cfg) {
			throw new Error("Empty config send to spawn");
		}
		if (!cfg.enabled) {
			log('warn', logSystem, `${mod} module is disabled via config`);
			return;
		}
			...
			switch(msg.type){
				...
				Object.keys(cluster.workers).forEach(function(id) {
					if (cluster.workers[id].type === 'poolWorker'){
						cluster.workers[id].send({type: 'validShare', validShare: msg.validShare, ip: msg.ip, login: msg.login});
					}
				});
				break;
				case 'banIP':
		...
		setTimeout(() => {
			let spawnInterval = setInterval(function(){
				...
				if (i -1 === cfg.clusterForks ){
					log('info', logSystem, 'Module %s spawned on %d thread(s)', [mod, cfg.clusterForks ]);
