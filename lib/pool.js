 ...
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        if (!addressType) {
           ...
           log('warn', logSystem, 'Invalid address used for login (prefix: %s): %s [w:%s]', [addressPrefix, login, monitorIPs[ip]]);
    ...
    if (!params.nonce || !params.result) {
       ...
       log('warn', logSystem, 'Invalid address used for login (prefix: %s): %s', [addressPrefix, login, monitorIPs[ip]]);
...
const que = async.queue(function(share, next) {
    ...
    log('info', logSystem, 'Que process %s share %d at difficulty %d from %s@%s', [share,shareType, share,job.difficulty, share.shareDiff, share.miner.login, share.miner.ip]);
