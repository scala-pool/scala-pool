 ...
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        if (!addressType) {
           ...
           log('warn', logSystem, 'Invalid address used for login (prefix: %s): %s:%s [w:%s]', [addressPrefix, login,ip, monitorIPs[ip] ?  monitorIPs[ip] : 'BAN']);
    ...
    if (!params.nonce || !params.result) {
        ...
        log('warn', logSystem, 'Malformed miner share: %s from (%s@%s)', [JSON.stringify(params), miner?miner.login:'', ip, monitorIPs[IP] ? monitorIPs[IP] : "BAN"])
        return;
    }
    ...
    const shareStats = processShare(miner, job, blockTemplate, params);
    const shareAccepted = shareStats.status || shareStats.error === 3;
    if(shareStats.error === 3) {
        miner.setNewDiff(miner.difficulty * 3);
        return;
    } 
    ...
    if (!shareAccepted){
        sendReply('Rejected share: ' + shareStats.msg);
        return;
    }
...
const que = async.queue(function(share, next) {
    
    log('info', logSystem, 'Que process share %d at difficulty %d from %s@%s', [ share.job.difficulty, share.blockTemplate.difficulty, share.miner.login, share.miner.ip]);
    rpcDaemon.submitBlock(share.shareBuffer.toString('hex'), function(error, result){
        if (error){
            log('error', logSystem, 'Error submitting block at height %d from %s@%s, share type: "%s" - %j', [share.job.height, share.miner.login, share.miner.ip, share.shareType, error]);
        }else{
            const blockFastHash = utils.cnUtil.get_block_id(share.shareBuffer, 0).toString('hex');
            log('info', logSystem, 'Block %s found at height %d by miner %s@%s - submit result: %j', [blockFastHash.substr(0, 6), share.job.height, share.miner.login, share.miner.ip, result]);
            recordShareData(share.miner, share.job, share.hashDiff.toString(), true, blockFastHash, share.shareType, share.blockTemplate);
        }
        next();
    });
}, 2);
 ...
 **/
 function processShare(miner, job, blockTemplate, params){
    ...
    const share = new Shares(miner, job, blockTemplate, params);
    if (!share.shareBuffer) {
        return {status:false, msg: 'No share buffer generated', error: 1};
    }
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) {
       share.hashDiff = hashDiff;
       que.push(share);
    } else if (hashDiff.lt(job.difficulty)) {
        log('warn', logSystem, 'Rejected low difficulty share of %s from %s@%s', [hashDiff.toString(), miner.login, miner.ip]);
        return {status:false, msg: 'Low difficulty share', error: 3};
    } else {
        recordShareData(miner, job, hashDiff.toString(), false, null, share.shareType);
    } 
        ...
        const socketResponder = function(socket){
            ...
            socket.on('data', function(d){
                ...
                if (Buffer.byteLength(dataBuffer, 'utf8') > 10240){ //10KB
                    dataBuffer = null;
                   log('warn', logSystem, 'Socket flooding detected and prevented from  %s: %s [w:%s]', [ip, message, monitorIPs[ip]]);
                    socket.destroy();
                    return;
                }
                if (dataBuffer.indexOf('\n') !== -1){
                    ...
                    for (var i = 0; i < messages.length; i++){
                        ...
                        try{
                            ...
                            log('warn', logSystem, 'Malformed message from %s: %s [w:%s] ', [ip, message, monitorIPs[ip] ? monitorIPs[ip] : "BAN"]);
                            socket.destroy();
                            break;
                        }
