...
const logSystem = 'poolWorker'
const BanManager = require('../model/BanManager');
...
function jobRefresh () {
	...
	async.series([
			next => {
				rpcDaemon.getLastBlockData((err, res) => {
					...
					if(blockHeader.height !== lastHash.lastblock_height || blockHeader.hash !== lastHash.lastblock_hash) {
						...
						log('info', logSystem, '%s found new hash %s at Heigh: %s', [global.config.coin, lastHash.lastblock_hash, lastHash.lastblock_height]);
						next(null);
						return
					}
					next(true)
		...
		function (e,r) {
			...
			BanManager.onInterval();
			timeoutInterval = setTimeout(jobRefresh, 1000)
		});
}
 ...
 **/ 
 ...
 process.on('message', function(message) {
    switch (message.type) {
        ...
        break;
        case 'validShare':
        BanManager.onIpCheck(message.ip, message.validShare);
        break;
    }
});
