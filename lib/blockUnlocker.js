...
function runInterval(){
    ...
    async.waterfall([
        function(callback) {
            const aCoins = [];
            async.eachSeries(Object.keys(CoinCollection), function(coin, next) {
                redisClient.hmget(coin + ':stats', ['lastblock_height','lastblock_hash'], (e,r) => {
                    if(e || !r[0] || !r[1] || (blockHeader[coin].height === r[0] && blockHeader[coin].hash === r[1])) {
                        callback(true);
                        return;
                    }
                    if(!blockHeader[coin]) {
                    ...
                    blockHeader[coin].height = r[0];
                    blockHeader[coin].hash = r[1];
                    aCoins.push(coin);
                    next(null);
                });
            }, function(error){
        ...
        function(blocks, callback){
            ...
            async.eachSeries(blocks, function(block, next){
                ...
                const limitPerTransaction = CoinCollection[block.coin].Config.unlocker.maxBlocks || 20

                if(!totalBlocks[block.coin]) {
                    totalBlocks[block.coin] = 0;
                }
                totalBlocks[block.coin]++;

                /* Limit blocking quatities */
                if(limitPerTransaction > 0 && limitPerTransaction <= unlockedBlocks[block.coin].length) {
                    if(!~warn.indexOf(block.coin)) {
                        log('warn',logSystem,`Maximum block per transaction is reached (${limitPerTransaction}) for coin ${block.coin}`)
                        warn.push(block.coin);
                    }
                    next(null);
                    return;
                }
                ...
                rpcDaemon.getBlockHeaderByHeight({height: blockHeight}, function(error, result){
                    ...
                    if(block.isDirty()) {
                        cmd.push(['zrem', block.coin + ':blocks:candidates', block.serialized]);
                        cmd.push(['zadd', block.coin + ':blocks:candidates', block.height, block.toRedis()]);

                        block.serialized = block.toRedis();
                    }

                    if(block.unlocked) {
                        if(!unlockedBlocks[block.coin]) {
                            unlockedBlocks[block.coin] = [];
                        }
                        unlockedBlocks[block.coin].push(block);
                    }
                ...
                for(let [coin, unlockBlock] of unlockedBlocks) {
                    if(cmd.length > 0) {
                        log('info', logSystem, 'Unlocking blocks for coin %s (%d pending | %d total | %d dirty)', [coin, unlockBlock.length, totalBlocks[coin], cmd.length/2]);
                        redisClient.multi(cmd).exec(endProcess)
                    } else if(Object.values(unlockBlock).length > 0){
