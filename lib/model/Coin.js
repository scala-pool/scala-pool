...
class Coin {
	...
	get unlockerReward() {
		const haveBlockUnlockerAward = this.Config.blockUnlocker.reward && this.Config.blockUnlocker.reward > 0;

		let fee = 0;

		if(fee) {
			fee = this.Config.blockUnlocker.reward;
		} 
	...
	get poolFee(type) {
		let poolFee = (this.Config.payments.poolFee && type in this.Config.payments.poolFee) ? global.config.payments.poolFee[type] : 0;
		let fee = this.Config.payments.poolFee || 0.00;
        if(fee > 1) {
        	fee /= 100;
        }
	...
	get networkFee() {
        ...
        let fee = this.Config.payments.networkFee || 0.00;
        if(fee > 1) {
        	fee /= 100;
        }
	...
	get devFee() {
        ...
        let fee = this.Config.payments.devFee || 0.00;
        if(fee > 1) {
        	fee /= 100;
        }
