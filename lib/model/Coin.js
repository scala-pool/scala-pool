...
class Coin {
	...
	constructor(config) {
		this.Share = require(path.join('..','pool','Shares' ,this.shareClassName));
		this.Config = config;

		if(!this.CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX) {
			this.CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX = parseInt(utils.cnUtil.address_decode(Buffer.from(config.poolServer.poolAddress)).toString());
		}
		if(!this.CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX) {
	...
	isWithPaymentId (address, separator) {
		separator = separator || this.Config.poolServer.paymentId.addressSeparator
